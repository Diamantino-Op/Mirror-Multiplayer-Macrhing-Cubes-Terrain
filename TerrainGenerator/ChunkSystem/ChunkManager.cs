...
public class ChunkManager : Singleton<ChunkManager>
{
    ...
    void CmdLoadRegion(int initX, int initZ)
    {
    ...
    void Update()
    {
        if (NetworkClient.ready && isInitialized)
        {
            if (isClient)
            {
                if (lastChunkViewDistance != chunkViewDistance)
                    CalculateDistances();
                CmdHiddeRemoveChunk(player, removeDistance, hideDistance);
                CmdCheckNewChunks(player);
                CheckRegion();
            }
        }
    ...
    /// </summary>
    [Command(requiresAuthority = false)]
    void CmdHiddeRemoveChunk(Transform sender, float rmDist, float hdDist)
    {
        List<Vector2Int> removeList = new List<Vector2Int>(); ;
        foreach (KeyValuePair<Vector2Int, Chunk> chunk in chunkDict)
        {
            float distance = Mathf.Sqrt(Mathf.Pow((sender.position.x - chunk.Value.transform.position.x), 2) + Mathf.Pow((sender.position.z - chunk.Value.transform.position.z), 2));
            if (distance > rmDist)
            {
                chunk.Value.saveChunkInRegion();//Save chunk only in case that get some modifications
                NetworkServer.Destroy(chunk.Value.gameObject);
                removeList.Add(chunk.Key);
            }
            else if (distance > hdDist && chunk.Value.gameObject.activeSelf)
            {
    ...
    /// </summary>
    [Command(requiresAuthority = false)]
    void CmdCheckNewChunks(Transform sender)
    {
        Vector2Int actualChunk = new Vector2Int(Mathf.CeilToInt((sender.position.x- Constants.CHUNK_SIDE / 2) / Constants.CHUNK_SIDE ),
                                                Mathf.CeilToInt((sender.position.z - Constants.CHUNK_SIDE / 2) / Constants.CHUNK_SIDE ));
    ...
    void LoadChunkFromList()
    {
        ...
        chunkObj.name = "Chunk_" + key.x + "|" + key.y;
